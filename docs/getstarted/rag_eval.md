...
!!! note "OpenAI Embeddings API"
    `ragas.embeddings.OpenAIEmbeddings` exposes `embed_text` (single) and `embed_texts` (batch), not `embed_query`/`embed_documents` like some LangChain wrappers. The example below uses `embed_texts` for documents and `embed_text` for the query. Please refer to [OpenAI embeddings implementation](../references/embeddings.md#ragas.embeddings.OpenAIEmbeddings)
